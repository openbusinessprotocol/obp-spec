...
- Owner が自分の Flap ID を別の主体に移譲する
- Owner が自分のエージェントに「自分自身の権限をさらに別のエージェントに委譲する権限」を与える
- flow.delegate の受任者が、授権者の同意なく第三者に再委譲する
```
...
flow.delegate プロトコルの構造自体が「再委譲の禁止」を実装する。
  - flow.delegate の成立条件 A: Owner が明示的に授権している
  - 受任者への「再委譲権限」は授権の対象外（プロトコル上不可）
